package com.example.bookSearch;

public class Constants {
    public final static String Contr_Search = "Search Controller" ; 
    public final static String Contr_Book = "Book Controller" ; 
    public final static String SVC_Search = "Search service" ; 
    public final static String SVC_Book = "Book service" ; 
    public static final String ENV_REDIS_PASSWORD="Redis_Pass_Weather";
    public final static String URL_Base = "http://openlibrary.org/search.json" ;
    public final static String URL_Pic_Base="https://covers.openlibrary.org/b/id/%s-L.jpg";
    public final static String URL_Find_One_Book_Base="https://openlibrary.org/works/%s.json";
    public final static String Bean_Book_Cache="Book cache";
    public final static String Repo_redis = "redis repo";
}
